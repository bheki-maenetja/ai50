...
def np_chunk(tree):
    ...
    for sub_t in parented_tree.subtrees(lambda t: t.label() == 'N'):
        right_sib = sub_t.right_sibling()
        if right_sib:
            if any(t for t in right_sib.subtrees(lambda t: t.label() == 'NP')):
                continue
        noun_chunks.append(sub_t.parent())
